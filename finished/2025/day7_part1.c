...
int main(void)
{
    ...
    for (size_t i=0; i < grid->rows; i++) {
        ...
        for (size_t j=0; j < grid->columns; j++) {
            ...
            tile->initial = line[j];
            tile->laser = tile->initial == 'S';
        }
    }

    int splits = 0;
    for (int i=0; i < (int)grid->rows; i++) {
        for (int j=0; j < (int)grid->columns; j++) {
            tile_t *tile = get_tile(grid, i, j);
            tile_t *below = get_tile(grid, i+1, j);
            printf("%c", (tile->laser ? '*' : tile->initial));
            if (tile->laser) {
                if (below->initial == '^') {
                    splits += 1;
                    tile_t *left = get_tile(grid, i+1, j-1);
                    left->laser = true;
                    tile_t *right = get_tile(grid, i+1, j+1);
                    right->laser = true;
                } else {
                    below->laser = true;
                }
